<script>
    ...
    const validationWrapper = async () => {
        ...
        if (validationRes.promoterDataValidation) {
            registerValidationData.promoterDataValidation =
                validationRes.promoterDataValidation;
        }

        if (validationRes.tourBusinessDataValidation) {
            registerValidationData.tourBusinessDataValidation =
                validationRes.tourBusinessDataValidation;
        }

        if (validationRes.tourOperatorDataValidation) {
            registerValidationData.tourOperatorDataValidation =
                validationRes.tourOperatorDataValidation;
        }

        if (validationRes.contentCreatorDataValidation) {
            registerValidationData.contentCreatorDataValidation =
                validationRes.contentCreatorDataValidation;
        }

        if (validationRes.conciergeDataValidation) {
            registerValidationData.conciergeDataValidation =
                validationRes.conciergeDataValidation;
        }
    };
</script>
    ...
    <div class="register-wrapper">
        <div class="register-wrapper__account-type-selection">
            <AccountTypeSelection
                {selectedAccount}
                onChangeSelection={onAccountTypeSelectionChange}
            />
        </div>
        ...
        <div class="register-wrapper__form-section">
            ...
            <div stlye={{ display: "flex", flexDirection: "row" }}>
                <button
                    ...
                    on:click={() => navigate("/login")}>Cancel</button
                >
                <button
                    class="main-button cta-button"
                    on:click={() => validationWrapper()}>Register</button
                >
            </div>
        </div>
    </div>
